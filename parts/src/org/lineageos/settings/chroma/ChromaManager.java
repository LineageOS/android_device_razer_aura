...
public class ChromaManager {
    ...
    public void applySettings(Context context) {
        ...
        String color = prefs.getString(ChromaFragment.KEY_COLOR, "#167C80");
        int brightness = prefs.getInt(ChromaFragment.KEY_BRIGHTNESS, 78);

        switch (mode) {
            case "color":
                applyColor(Color.parseColor(color));
                break;
            case "breath":
                applyProgram(PROGRAM_BREATHING, "03", BREATHING_START_2, BREATHING_START_3);
                break;
            case "spectrum":
                applyProgram(PROGRAM_SPECTRUM, "03", SPECTRUM_START_2, SPECTRUM_START_3);
                break;
            case "wave_ltr":
                applyProgram(PROGRAM_WAVE_LTR, WAVE_START_1, WAVE_START_2, WAVE_START_3);
                break;
            case "wave_rtl":
                applyProgram(PROGRAM_WAVE_RTL, WAVE_START_1, WAVE_START_2, WAVE_START_3);
                break;
        }
    ...
    public void setBrightness(int percent) {
        // percent is 1–100; map to hardware range 5–255
        int value = 5 + Math.round((Math.max(1, Math.min(100, percent)) - 1) * 250f / 99f);
        synchronized (mLock) {
            try {
